...
export function Dashboard() {
  const { getDashboardStats, isLoading } = useProductStore();
  const { openModal } = useUIStore();
  const [activeModal, setActiveModal] = useState<ActiveModal>(null);
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="bg-white rounded-2xl border border-beige-100 shadow-sm p-5">
            ...
            <div className="flex flex-wrap gap-3">
              {stats.outOfStockProducts > 0 && (
                <button
                  onClick={() => setActiveModal('out-of-stock')}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-50 border border-red-100 hover:bg-red-100 transition-colors cursor-pointer"
                >
                  <XCircle size={15} className="text-red-500" />
                  <span className="text-sm font-medium text-red-700">
                    {stats.outOfStockProducts} product{stats.outOfStockProducts > 1 ? 's' : ''} out of stock
              ...
              {stats.lowStockProducts > 0 && (
                <button
                  onClick={() => setActiveModal('low-stock')}
                  className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-amber-50 border border-amber-100 hover:bg-amber-100 transition-colors cursor-pointer"
                >
                  <AlertTriangle size={15} className="text-amber-500" />
                  <span className="text-sm font-medium text-amber-700">
                    {stats.lowStockProducts} product{stats.lowStockProducts > 1 ? 's' : ''} running low
      ...
      {stats.totalProducts === 0 && (
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.35 }}
          className="bg-white rounded-2xl border border-beige-100 shadow-sm"
        >
          <EmptyState
            icon={<Package size={28} />}
            title="No products yet"
      ...
      {/* Modals */}
      <TotalProductsModal open={activeModal === 'total-products'} onClose={() => setActiveModal(null)} />
      <TotalInventoryModal open={activeModal === 'total-inventory'} onClose={() => setActiveModal(null)} />
      <LowStockModal open={activeModal === 'low-stock'} onClose={() => setActiveModal(null)} />
      <OutOfStockModal open={activeModal === 'out-of-stock'} onClose={() => setActiveModal(null)} />
    </div>
  );
}