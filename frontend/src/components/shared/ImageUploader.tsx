    ...
    async (acceptedFiles: File[]) => {
      ...
      try {
        const results = await Promise.all(
          filesToProcess.map((file) => uploadsApi.uploadImage(file))
        );
        ...
        >
          ...
          <div className="flex flex-col items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-sage-400 mt-0.5">
                or <span className="text-sage-500 underline">browse files</span>
                {' · '}JPG, PNG, WebP
              </p>
            </div>
            {images.length > 0 && (
              <p className="text-xs text-sage-400">{images.length}/{maxImages} images</p>
            )}
          </div>
        </div>
      )}
            ...
            {images.map((image, index) => (
              ...
              >
                ...
                <button
                  type="button"
                  onClick={(e) => { e.stopPropagation(); removeImage(index); }}
                  className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-red-500 text-white rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-sm hover:bg-red-600 cursor-pointer"
                >
                  <X size={10} />
                </button>
