...
from ..models.product import (
    ProductCreate, ProductUpdate, ProductResponse, SizeInventory
)
from ..middleware.auth import get_current_user
from ..database.mongodb import get_database
from datetime import datetime
...
# Only fetch fields we actually need — reduces network + deserialization time
PRODUCT_PROJECTION = {
    "_id": 1,
    "user_id": 1,
    "name": 1,
...
async def create_product(
    ...
    # Auto-create production + cloth entries in parallel
    production_doc = {
        "user_id": ObjectId(current_user["_id"]),
        "product_id": product_id,
        "name": product_data.name,
    ...
    await asyncio.gather(
        db.production.insert_one(production_doc),
        db.cloths.insert_one(cloth_doc),
    )
...
async def get_products(
    ...
    if search:
        # Use regex for partial match — covered by (user_id, name text) index
        query["name"] = {"$regex": search, "$options": "i"}
    ...
    products = await cursor.to_list(length=500)  # cap at 500 for performance
    return [product_response(p) for p in products]
...
async def get_dashboard_stats(
    ...
    # Use aggregation pipeline — single DB round trip instead of fetching all docs
    pipeline = [
        {"$match": {"user_id": ObjectId(current_user["_id"])}},
        {
            "$group": {
                ...
                "total_inventory": {"$sum": {"$sum": "$sizes.quantity"}},
                "low_stock": {
                    "$sum": {
                        "$cond": [
                            {
    ...
    results = await db.products.aggregate(pipeline).to_list(length=1)
    if not results:
        return {"total_products": 0, "total_inventory": 0, "low_stock": 0, "out_of_stock": 0}
...
async def update_product(
    ...
    # Sync name + images to production and cloths in parallel
    sync: dict = {}
    if product_data.name is not None:
        sync["name"] = product_data.name
    if product_data.images is not None:
    ...
    if sync:
        ...
        await asyncio.gather(
            db.production.update_many(
                {"product_id": pid, "user_id": uid}, {"$set": sync}
            ),
            db.cloths.update_many(
                {"product_id": pid, "user_id": uid}, {"$set": sync}
            ),
        )
...
async def delete_product(
    ...
    # Return 204 even if product was already deleted (idempotent delete)
    # This prevents the "404 on second click" issue on the frontend
    if result.deleted_count == 0:
        # Check if it exists at all — if not, it was already deleted, return 204
        exists = await db.products.count_documents({"_id": ObjectId(product_id)})
        if exists == 0:
            # Already deleted — clean up related docs just in case, return success
            pass
        else:
            raise HTTPException(status_code=403, detail="Not authorized to delete this product")

    # Delete linked production + cloth entries in parallel
    import asyncio
    uid = ObjectId(current_user["_id"])
    pid = ObjectId(product_id)
    await asyncio.gather(
        db.production.delete_many({"product_id": pid, "user_id": uid}),
        db.cloths.delete_many({"product_id": pid, "user_id": uid}),
    )
