...
# # def production_response(production: dict) -> ProductionResponse:
# #     """Convert production document to response"""
# #     total_inventory = sum(size["quantity"] for size in production.get("sizes", []))
    
# #     return ProductionResponse(
# #         id=str(production["_id"]),
# #         user_id=str(production["user_id"]),
# #         name=production["name"],
# #         category=production["category"],
# #         images=production.get("images", []),
# #         sizes=[SizeInventory(**size) for size in production.get("sizes", [])],
...
# #         "images": production_data.images,
# #         "sizes": [size.dict() for size in production_data.sizes],
# #         "created_at": datetime.utcnow(),
# #         "updated_at": datetime.utcnow()
# #     }
    
# #     result = await db.production.insert_one(production_doc)
# #     production_doc["_id"] = result.inserted_id
    
# #     return production_response(production_doc)
...
# #     current_user: dict = Depends(get_current_user)
# # ):
# #     """Get all production entries for current user"""
# #     db = get_database()
...
# #     cursor = db.production.find(query).sort("created_at", -1)
# #     productions = await cursor.to_list(length=1000)
    
# #     return [production_response(production) for production in productions]
...
# #     if not ObjectId.is_valid(production_id):
# #         raise HTTPException(
# #             status_code=status.HTTP_400_BAD_REQUEST,
# #             detail="Invalid production ID"
# #         )
    
# #     production = await db.production.find_one({
...
# #     return production_response(production)
...
# #     if production_data.name is not None:
# #         update_data["name"] = production_data.name
# #     if production_data.category is not None:
# #         update_data["category"] = production_data.category
# #     if production_data.images is not None:
# #         update_data["images"] = production_data.images
# #     if production_data.sizes is not None:
# #         update_data["sizes"] = [size.dict() for size in production_data.sizes]
    
# #     # Update production
# #     result = await db.production.find_one_and_update(
# #         {
# #             "_id": ObjectId(production_id),
# #             "user_id": ObjectId(current_user["_id"])
# #         },
...
# #     return production_response(result)
...
# #     result = await db.production.delete_one({
# #         "_id": ObjectId(production_id),
# #         "user_id": ObjectId(current_user["_id"])
# #     })
...
#     production_doc = {
#         "user_id": ObjectId(current_user["_id"]),
#         "product_id": ObjectId(production_data.product_id) if production_data.product_id else None,
#         "name": production_data.name,
#         "category": production_data.category,
#         "images": production_data.images,
#         "sizes": [size.dict() for size in production_data.sizes],
...
#     cursor = db.production.find(query).sort("created_at", -1)
#     productions = await cursor.to_list(length=1000)
#     return [production_response(p) for p in productions]
...
#     # Only sizes can be updated by the user - name/category come from product
#     if production_data.sizes is not None:
#         update_data["sizes"] = [size.dict() for size in production_data.sizes]
#     if production_data.images is not None:
#         update_data["images"] = production_data.images
...
#     result = await db.production.find_one_and_update(
#         {"_id": ObjectId(production_id), "user_id": ObjectId(current_user["_id"])},
#         {"$set": update_data},
#         return_document=True
#     )
#     if not result:
...
# async def delete_production_by_product(
#     product_id: str,
#     current_user: dict = Depends(get_current_user)
# ):
#     """Delete all production entries linked to a product (called on product deletion)."""
#     db = get_database()
...
#     current_user: dict = Depends(get_current_user)
# ):
#     """Internal use - called when product is deleted."""
#     db = get_database()
#     if not ObjectId.is_valid(production_id):
#         raise HTTPException(status_code=400, detail="Invalid production ID")
#     result = await db.production.delete_one({
